...
import sculpture from '@/assets/BlackBig_Chair.jpg';
import BigMirror from '@/assets/mirror.jpg';
import BottleNose from '@/assets/BottleNose.jpg';
import Lamp from '@/assets/Lamp_Candles.jpg';
import Avacado from '@/assets/avacado_mirror.jpg';
import Flower from '@/assets/flower_lamp.jpg';
...
export default function CollectionSection() {
  ...
  return (
    <section ref={ref} className="w-full min-h-[75vh] md:min-h-screen flex flex-col items-center justify-center bg-white dark:bg-black px-2 md:px-8 pb-12 relative overflow-hidden">
      {/* COLLECTION Heading */}
      <h1 className="font-cinzel text-black dark:text-white text-4xl sm:text-5xl md:text-6xl text-center my-8 md:my-24 tracking-wider select-none transition-colors duration-300">
        COLLECTION
      </h1>
        ...
        <div className="flex flex-row w-full gap-4 mb-4">
          ...
          <div className="w-1/2 flex items-center justify-center">
            <div className="aspect-[3/4] w-full max-w-[180px] rounded-lg overflow-hidden shadow-lg mx-auto">
              <Image src={sculpture} alt="Sculpture" className="object-cover w-full h-full" priority />
            </div>
          </div>
          {/* Text */}
          <div className="w-1/2 flex flex-col justify-center items-start pr-2">
            <h2 className="text-xl font-cinzel text-black dark:text-white mb-2 tracking-wider select-none transition-colors duration-300">SCULPTURES</h2>
            <p className="amiko-regular text-black dark:text-white text-xs sm:text-sm text-left select-none transition-colors duration-300">
              war and peace, love and hate; although co–existing in the soul of the same being, it is always apure contentment.
            </p>
          </div>
        </div>
        {/* Carousel grid */}
        <div className="flex flex-row gap-4 overflow-x-auto pb-2 snap-x snap-mandatory w-full px-2">
          {[BigMirror, BottleNose, Lamp, Avacado, Flower, Round_Lamp].map((img, i) => (
            <div key={i} className="relative min-w-[160px] max-w-[180px] h-44 rounded-lg overflow-hidden flex-shrink-0 snap-center cursor-pointer hover:scale-105 transition duration-300">
              <Image src={img} alt={`Collection ${i}`} fill className="object-cover" />
              {i === 5 && (
      ...
      <div className="hidden md:flex relative flex-1 w-full flex-col md:flex-row items-stretch justify-center max-w-[1600px] mx-auto" style={{ minHeight: columnHeight }}>
        ...
        <div
          className={`
            ...
            transform
            ${animate ? '-translate-x-1/2 -translate-y-1/2 md:translate-x-0 md:translate-y-0' : '-translate-x-1/2 -translate-y-1/2 md:translate-x-1/2 md:translate-y-0'}
            transition-all duration-1000 ease-in-out
            z-10
            flex items-center
          `}
          style={{ width: '90vw', maxWidth: '520px', height: columnHeight, maxHeight: 800 }}
        >
          <Image
            src={sculpture}
            alt="Sculpture"
        ...
        <div
          ...
          style={{ 
            height: columnHeight,
            transitionDelay: animate ? '500ms' : '0ms'
          }}
        >
          <h2 className="text-3xl md:text-4xl font-cinzel text-black dark:text-white mb-4 tracking-wider select-none transition-colors duration-300">SCULPTURES</h2>
          <p className="amiko-regular text-black dark:text-white text-base md:text-lg mb-8 select-none transition-colors duration-300">
            WAR AND PEACE, LOVE AND HATE; ALTHOUGH CO–EXISTING IN THE SOUL OF THE SAME BEING, IT IS ALWAYS A PURE HEART THAT ACHIEVES CONTENTMENT.
          </p>
          {/* Grid of images */}
          <div className="grid grid-cols-3 gap-12 w-full h-[calc(100%-7rem)] items-stretch">
            <div 
              className={`relative w-full h-full min-h-[120px] cursor-pointer hover:scale-120 transition-all duration-500 ${
            ...
            <div 
              ...
              style={{ transitionDelay: animate ? '900ms' : '0ms' }}
            >
              <Image src={BottleNose} alt="Bottle Nose" fill className="rounded-lg object-cover" />
            </div>
            <div 
              className={`relative w-full h-full min-h-[120px] cursor-pointer hover:scale-120 transition-all duration-500 ${
                animate ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
            ...
            >
              <Image src={Lamp} alt="Lamp" fill className="rounded-lg object-cover" />
            </div>
            <div 
              className={`relative w-full h-full min-h-[120px] cursor-pointer hover:scale-120 transition-all duration-500 ${
                animate ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
              }`}
              style={{ transitionDelay: animate ? '1100ms' : '0ms' }}
            >
              <Image src={Avacado} alt="Avacado" fill className="rounded-lg object-cover" />
            </div>
            <div 
              className={`relative w-full h-full min-h-[120px] cursor-pointer hover:scale-120 transition-all duration-500 ${
                animate ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
            ...
            >
              <Image src={Flower} alt="Flower" fill className="rounded-lg object-cover" />
            </div>
            {/* Last image with overlay */}
            <div 
              className={`relative w-full h-full min-h-[120px] group transition-all duration-500 ${
                animate ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
              }`}
              style={{ transitionDelay: animate ? '1300ms' : '0ms' }}
            >
